...
def main(args):
    ...
    image_path = [path for path in os.listdir(args.images_path) if 'CAPTIONED' not in path and '.jpg' in path][0]
    image = transform(Image.open(f'{args.images_path}/{image_path}'))
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ])
    sample = torch.stack([transform(image.convert('RGB'))], dim=0)
    ...
    with torch.no_grad():
        ...
        system_tokens = [dictionary.words[tok] for tok in hypos[0][0]['tokens'] if tok != dictionary.eos_idx]
